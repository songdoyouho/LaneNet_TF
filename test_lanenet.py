...
def test_lanenet(image_path, weights_path, use_gpu):
    ...
    with sess.as_default():
        ...
        np.save("binary.npy", binary_seg_image)
        ...
        np.save("instance.npy", instance_seg_image) 
        for i in range(4):
            instance_seg_image[0][:, :, i] = minmax_scale(instance_seg_image[0][:, :, i])

           
        embedding_image = np.array(instance_seg_image[0], np.uint8)
        print(embedding_image[:,:,3])
        print(embedding_image[:,:,2])
        print(embedding_image[:,:,1])
        ...
        cv2.imwrite("embedding_image.png", embedding_image)
